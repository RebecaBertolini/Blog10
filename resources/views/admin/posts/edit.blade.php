        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-5">
                <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    @method('PUT')
                    {{-- O fomulario so entende post, mas o metodo de restfull das routes precisa que seja no padrao put pois há uma atualizacao --}}
                    <div class="w-full mb-6">
                        ...
                        @enderror
                        <input type="text" class="w-full dark:bg-gray-400 rounded" name="title" value="{{$post->title}}">
                    </div>
                    <div class="w-full mb-6">
                        <label for="" class="block text-white my-4">Descrição</label>
                        <input type="text" class="w-full dark:bg-gray-400 rounded" name="description" value="{{$post->description}}">
                    </div>
                    <div class="w-full mb-6">
                        ...
                        @enderror
                        <input type="text" class="w-full dark:bg-gray-400 h-24 flex items-start rounded" rows="5" name="body" value="{{$post->body}}">
                    </div>
                    <div class="w-full mb-6">
                    ...
                    <div class="w-full mb-6">
                        <label for="" class="block text-white my-4">Status</label>
                        <div class="flex gap-5 mt-5">
                            <input type="radio" class="" name="is_active" value="1" @if($post->is_active) checked @endif> Ativo
                            <input type="radio" class="" name="is_active" value="0" @if(!$post->is_active) checked @endif> inativo
                        </div>
                    </div>
                    <button class="text-white px-4 py-2 shadow text-white text-bold bg-fuchsia-800 hover:bg-fuchsia-600 rounded transition ease-in-out durantion-300">Editar postagem</button>
                </form>
